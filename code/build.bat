REM
REM  (COPYRIGHT) Copyright (C) 2008, 2009, The ONS Team.
REM  This file is part of ONS, see COPYING for details.
REM

REM
REM File information:
REM - Created: 15. March 2009
REM - Lead-Dev: - David Herrmann
REM - Contributors: /
REM - Last-Change: 15. March 2009
REM

REM
REM This file buils the ONS library on WINDOWS systems.
REM


@echo off
echo Error: This BUILD script is currently under development.

